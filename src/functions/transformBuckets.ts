...
export function transformBucketsData(
  ...
  const salaryBucketIndex = buckets.findIndex((bucket, _index, buckets) => {
    ...
    const isWithinABucket = salary >= currentBucketMinMax.min && salary < currentBucketMinMax.max
    const isBetweenTwoBucket = salary > prevBucketMinMax.max && salary < nextBucketMinMax.min
  ...
  const transformedBuckets: [string, number][] = buckets.map((bucket) => {
    ...
    if (type === 'percentages' && stats?.totalCount) {
      value = Number(Number((bucket.count / stats.totalCount) * 100).toPrecision(3))
    } else {
      value = bucket.count
    }
